package org.fengye.commonview.lib.starview;
...
public class StarView extends View {
    ...
    private void init(Context mContext, AttributeSet attrs) {
        ...
        starNum = typedArray.getInteger(R.styleable.StarView_starsNum, 5);
        starHeight = (int) typedArray.getDimension(R.styleable.StarView_starHeight, 24);
        starWidth = (int) typedArray.getDimension(R.styleable.StarView_starWidth, 24);

        starDistance = (int) typedArray.getDimension(R.styleable.StarView_starDistance, 0);
        isClick = typedArray.getBoolean(R.styleable.StarView_starClickable, true);
        starBackgroundBitmap = typedArray.getDrawable(R.styleable.StarView_starBackground);

        Drawable starDrawBitmap = typedArray.getDrawable(R.styleable.StarView_starDrawBackground);

        typedArray.recycle();
        setClickable(isClick);
        if (starBackgroundBitmap == null) {
            starBackgroundBitmap = ContextCompat.getDrawable(getContext(), R.drawable.ic_star_bg);
        }
        if (starDrawBitmap == null) {
            starDrawBitmap = ContextCompat.getDrawable(getContext(), R.drawable.ic_star);
        }
        starDrawDrawable = drawableToBitmap(starDrawBitmap);
