package org.fengye.commonview.lib.timingview;
...
public class CountDownTextView extends ShapeTextView implements LifecycleObserver, View.OnClickListener {
    ...
    public static String generateTime(long time) {
        ...
        if (hours > 0) {
            format = String.format(Locale.getDefault(), "%02d时%02d分%02d秒", hours, minutes, seconds);
        } else if (minutes > 0) {
            format = String.format(Locale.getDefault(), "%02d分%02d秒", minutes, seconds);
        } else {
            format = String.format(Locale.getDefault(), "%2d秒", seconds);
        }
        return format;
    }
}